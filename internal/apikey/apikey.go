...
// vulos-meet admin API to authenticate `Authorization: Bearer vk_…` credentials.
//
// Introspection is brokered by the vulos-cloud control plane ("CP"). Every
// Vulos product presents the opaque key to the SAME endpoint and gets back the
// resolved account + scopes + products (identical contract to vulos-office):
//
...
//	            "scopes":  ["meet.rooms.read","meet.rooms.delete"],
//	            "products":["meet","mail"]}
//	  200  →   {"valid": false}                     (unknown/revoked/expired key)
//
// Results are cached in-process for cacheTTL (~60s) so a burst of admin API
// calls does not hammer the CP and a single key is introspected at most once
// per minute.
//
// This package imports nothing from internal/wrap: it is the wire seam, so
...
const (
	...
	// Bearer credential. A token without this prefix is treated as the static
	// admin bearer token.
	KeyPrefix = "vk_"

	// ProductMeet is the product scope a key MUST carry to use the meet admin
	// API. The CP returns the products a key is entitled to in `products`.
	ProductMeet = "meet"

	// EnvCPBaseURL points at the control-plane base URL (e.g. https://cp.vulos.to).
	// When unset the vk_ key path is disabled and meet falls back to the static
	// MEET_ADMIN_TOKEN auth (self-host unchanged). This is the SAME env var the
	// vulos-office introspection seam uses.
	EnvCPBaseURL = "VULOS_CP_BASE_URL"

	// EnvCPToken is the service token meet presents to the CP on the
	...
	// promptly, long enough to absorb a burst of admin API calls.
...
// Enabled reports whether the vk_ key path is configured (a CP base URL is
// set). When false the caller falls back to static admin-token auth.
